...
- **Refinement:** after the initial concept is accepted, changing an object is a new Generation. The user selects the object, marks the exact edit rectangle and states the requested change. The generation uses the last accepted scene, preserves its camera, and uses deterministic masked composition; every pixel outside the allowed edit rectangle is restored from the accepted input scene.
