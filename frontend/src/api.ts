...
export function adminCreateGenerationFlyover(payload: { source_generation_id: string; model_name: string; prompt: string; params: Record<string, unknown>; duration_seconds: number }) {
  return request<Generation>('/admin/generation/flyover', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
}
