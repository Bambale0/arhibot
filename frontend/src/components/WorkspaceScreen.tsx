...
import { appendDemoHistory } from '../history'
...
export function WorkspaceScreen({ project, initialMode, onBack, onProjectChange }: { project: Project; initialMode?: GenerationMode; onBack: () => void; onProjectChange: (project: Project) => void }) {
  ...
  async function runDemo() {
    if (!asset) return
    if (!demoEnabled) {
      setError('Интерфейс готов. Для реального результата нужен Generation API и AI-провайдер.')
      return
    }
    setError(null)
    setDemoBusy(true)
    await new Promise((resolve) => setTimeout(resolve, 1200))
    const result: DemoGeneration = { id: crypto.randomUUID(), mode, prompt, sourceUrl: asset.url, projectName: project.name, createdAt: new Date().toISOString() }
    setDemoResult(result)
    appendDemoHistory(result)
    setDemoBusy(false)
  }
  ...
  return (
    <main className="workspace-shell">
      <aside className="workspace-sidebar">
        ...
        <div className="sidebar-progress"><span>Рабочий процесс</span><div className="progress-item done"><b>1</b><div><strong>Проект</strong><small>Создан</small></div></div><div className={`progress-item ${asset ? 'done' : 'current'}`}><b>2</b><div><strong>Исходник</strong><small>{asset ? 'Загружен' : 'Нужен референс'}</small></div></div><div className={`progress-item ${asset ? 'current' : ''}`}><b>3</b><div><strong>Концепт</strong><small>Сценарий и пожелания</small></div></div></div>
      ...
      <section className="workspace-main">
        ...
        <div className="workspace-content">
          ...
          <section className="work-section"><div className="section-number">03</div><div className="section-body"><div className="section-title"><h3>Пожелания</h3><span>Необязательно</span></div><textarea className="prompt-input" value={prompt} onChange={(e) => {setPrompt(e.target.value); setDemoResult(null)}} placeholder="Например: современный минимализм, натуральные материалы, три спальни, приватная терраса, мягкий вечерний свет…" maxLength={2000}/><div className="prompt-footer"><span>{prompt.length}/2000</span><button onClick={() => setPrompt('Современный минимализм, натуральные материалы, спокойная палитра, функциональная планировка и реалистичная подача.')}>Заполнить пример</button></div></div></section>

          {error && <div className="banner-error workspace-error">{error}<button onClick={() => setError(null)}>Закрыть</button></div>}

          {demoResult && <section className="demo-result"><div className="demo-result-head"><div><span className="eyebrow">SANDBOX RESULT</span><h3>Результат сохранён в истории</h3></div><span className="status-pill">Тест</span></div><div className="demo-result-image"><img src={demoResult.sourceUrl} alt="Демонстрационный результат"/><div className="demo-watermark">AUROOM SANDBOX · здесь будет AI output</div></div><p>В песочнице показывается исходник, чтобы проверить полный пользовательский flow. Реальная AI-генерация требует подключения Generation API.</p></section>}

          <div className="generate-bar"><div><SparkIcon/><span><strong>{asset ? 'Всё готово для генерации' : 'Сначала добавьте исходник'}</strong><small>{demoEnabled ? 'AuRoom sandbox: результат попадёт в историю' : 'Требуется Generation API'}</small></span></div><button className="primary-button generate-button" disabled={!asset || demoBusy} onClick={() => void runDemo()}>{demoBusy ? 'Готовим…' : demoEnabled ? 'Запустить тест' : 'Создать'} <SparkIcon/></button></div>
        </div>
      </section>
    </main>
  )
