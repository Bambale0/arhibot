...
function GenerationPanel({ settings, prices, prompts, onSettings, onPrices, onPrompts, onError }: { settings: AdminGenerationSettings; prices: AdminGenerationPrice[]; prompts: AdminPrompt[]; onSettings:(v:AdminGenerationSettings)=>void; onPrices:(v:AdminGenerationPrice[])=>void; onPrompts:(v:AdminPrompt[])=>void; onError:(v:string|null)=>void }) {
  ...
  const [flyoverDuration,setFlyoverDuration]=useState('90')
  ...
  async function runFlyover(){
    ...
    if (
      !Number.isInteger(keyframeCount)||keyframeCount<4||keyframeCount>8
      ||!Number.isInteger(inbetweenFrames)||inbetweenFrames<0||inbetweenFrames>5
      ||!Number.isInteger(frameDuration)||frameDuration<50||frameDuration>500
    ) {
      onError('Flyover: 4–8 ключевых кадров, 0–5 промежуточных, 50–500 мс на кадр')
      return
    }
  ...
  return <section className="admin-panel"><div className="admin-panel-title"><div><h2>AI, стоимость и промпты</h2><p>Модели, параметры, стоимость кредитов и prompt templates управляются из БД.</p></div></div>
    ...
    <div className="admin-subpanel">
      ...
      {sandboxGeneration?.status==='completed'&&sandboxGeneration.output_asset&&<div className="admin-subpanel">
        <div className="admin-panel-title"><div><h3>Bird flyover GIF</h3><p>Дешёвый image-to-image пролёт: каждый новый ключевой кадр строится от предыдущего, затем промежуточные кадры дорисовываются локально и собираются в GIF. Видео-модель не вызывается.</p></div></div>
        <div className="admin-form-grid">
          ...
          <label>мс / кадр<input type="number" min="50" max="500" value={flyoverDuration} onChange={e=>setFlyoverDuration(e.target.value)}/></label>
          <label className="admin-span-2">Доп. инструкция<textarea value={flyoverPrompt} onChange={e=>setFlyoverPrompt(e.target.value)} placeholder="Например: сохранить мягкий вечерний свет"/></label>
          ...
          <div className="admin-span-2"><small>{Math.max(0,(Number(flyoverKeyframes)||0)-1)} новых image-вызовов + исходный кадр · {Math.max(0,((Number(flyoverKeyframes)||0)-1)*(Number(flyoverInbetweens)||0))} локальных промежуточных кадров · 0 video-вызовов · 0 кредитов AuRoom</small></div>
          <div className="admin-form-actions"><button type="button" className="secondary-button" disabled={flyoverBusy} onClick={()=>void runFlyover()}>{flyoverBusy?'Собираем GIF…':'Сгенерировать flyover GIF'}</button></div>
        </div>
