...
import type { DesignSession, QuestionnaireAnswer, QuestionnaireCatalog, QuestionnaireCondition, QuestionnaireDefinition, QuestionnaireProjectContext, QuestionnaireQuestion } from '../questionnaireTypes'
import type { Asset, Generation, GenerationMode, Project, ProjectContext } from '../types'
...
function newSession(version:string, selected:string[]):DesignSession {
  return { catalog_version:version, selected_objects:selected, current_object:selected[0] || null, current_question_id:null, source_step_completed:false, source_asset_id:null, scene_asset_id:null, answers:{}, accepted_objects:[], generation_ids:{}, edit_question_ids:[], review_comments:{}, application_submitted:false }
}
...
export function QuestionnaireWorkspaceScreen({ project, selectedObjects, onBack, onProjectChange }:{ project:Project; selectedObjects:string[]; onBack:()=>void; onProjectChange:(project:Project)=>void }) {
  ...
  useEffect(() => { let stop=false; (async () => {
    try {
      const loaded = await getQuestionnaireCatalog(); if (stop) return; setCatalog(loaded)
      ...
      const stored = (project.context as ProjectContext & QuestionnaireProjectContext).design_session
      const initial = stored && stored.catalog_version === loaded.version && stored.selected_objects.join('|') === selected.join('|') && !stored.application_submitted ? stored : newSession(loaded.version, selected)
      setSession(initial)
      if (initial.source_asset_id) try { setSourceAsset(await api.getAsset(initial.source_asset_id)) } catch { /* deleted source */ }
      const generationId = initial.current_object ? initial.generation_ids[initial.current_object] : null
      if (generationId) try { const generation = await api.getGeneration(generationId); if (generation.output_asset) setRenderOutput(generation.output_asset) } catch { /* stale generation */ }
    } catch (err) { setError(err instanceof Error ? err.message : 'Не удалось открыть опросник') }
  })(); return () => { stop=true } }, [project.id])
  ...
  async function persist(next:DesignSession) {
    setSession(next); setBusy(true)
    try {
      const context = { ...project.context, design_session:next } as ProjectContext
      const updated = await api.updateProject(project.id, { context }); onProjectChange(updated)
    } catch (err) { setError(err instanceof Error ? err.message : 'Не удалось сохранить ответы') }
  ...
  async function setSource(asset:Asset|null) {
    ...
    await persist({ ...session, source_step_completed:true, source_asset_id:asset?.id || null, scene_asset_id:asset?.id || null })
  }
  ...
  function prompt(definition:QuestionnaireDefinition, next:DesignSession) {
    const answers = next.answers[definition.key] || {}
    const lines = preQuestions(definition, next).map((q) => answers[q.id] == null || text(answers[q.id]) === '' ? null : `${q.id}. ${q.text} — ${text(answers[q.id])}`).filter(Boolean)
    const scene = next.scene_asset_id || next.source_asset_id ? 'Используй исходное изображение как текущую сцену. Сохрани существующий дом и все уже принятые объекты, их геометрию, пропорции, положение, окружение, ракурс и свет. Добавь или измени только текущий объект.' : definition.key === 'eskez-doma' ? 'Создай внешний вид дома на участке. Камера: дрон 40–50 м, сверху угловой вид.' : 'Создай объект на участке. Камера: дрон сверху, угловой вид.'
    return [`AuRoom. Точный опросник: ${definition.title}.`, scene, definition.key === 'eskez-doma' ? 'Планировок, комнат и внутренних помещений не придумывать.' : 'Если выбран «Как у дома», наследуй стиль, материалы и кровлю принятого дома.', ...lines, next.review_comments[definition.key] ? `Комментарий к уточнению: ${next.review_comments[definition.key]}` : ''].filter(Boolean).join('\n')
  }

  async function poll(generation:Generation) {
    let currentGeneration = generation
    for (let i=0; i<90 && ['queued','processing'].includes(currentGeneration.status); i++) { await delay(2000); currentGeneration = await api.getGeneration(currentGeneration.id) }
    if (currentGeneration.status !== 'completed' || !currentGeneration.output_asset) throw new Error(currentGeneration.error || 'Генерация не завершилась')
    return currentGeneration
  }

  async function generate(next:DesignSession, definition:QuestionnaireDefinition) {
    setBusy(true); setError(null); setRenderOutput(null)
    try {
      ...
      const generation = await poll(await api.createGeneration({ project_id:project.id, input_asset_id:input, type:mode, prompt:prompt(definition, next) }))
      setRenderOutput(generation.output_asset)
      const review = definition.questions.find((q) => q.phase === 'review')
      await persist({ ...next, generation_ids:{ ...next.generation_ids, [definition.key]:generation.id }, current_question_id:review?.id || null })
    } catch (err) {
  ...
  async function answer(question:QuestionnaireQuestion, value:QuestionnaireAnswer) {
    ...
    const nextAnswers = { ...session.answers, [current.key]:{ ...(session.answers[current.key] || {}), [question.id]:value } }
    let next:DesignSession = { ...session, answers:nextAnswers }
    ...
    if (current.key === 'zayavka') {
      ...
      if (question.id === '25' && value === true) return persist({ ...next, application_submitted:true, current_question_id:null })
      return
    }

    if (next.edit_question_ids.includes(question.id)) {
      const rest = next.edit_question_ids.filter((id) => id !== question.id)
    ...
    if (current.key === 'eskez-doma' && question.id === '15') {
      ...
      const q = current.questions.find((item) => item.id === '15а'); return persist({ ...next, current_question_id:q?.id || null })
    }
    if (current.key === 'eskez-doma' && question.id === '15а') {
      ...
      const q = current.questions.find((item) => item.id === '15б'); return persist({ ...next, current_question_id:q?.id || null })
    }
    if (current.key === 'eskez-doma' && question.id === '15б' && Array.isArray(value)) {
      ...
      if (!ids.length) return generate(next, current)
  ...
  async function accept(next:DesignSession, definition:QuestionnaireDefinition) {
    if (!renderOutput) { setError('Нет готового эскиза для принятия.'); return }
    const accepted = next.accepted_objects.includes(definition.key) ? next.accepted_objects : [...next.accepted_objects, definition.key]
    ...
    await persist({ ...next, accepted_objects:accepted, scene_asset_id:renderOutput.id, current_object:nextKey, current_question_id:first?.id || null, edit_question_ids:[] })
  }
  ...
  if (session.application_submitted) return <main className="questionnaire-shell"><section className="questionnaire-card finish-card"><SparkIcon/><span className="eyebrow">ГОТОВО</span><h1>Заявка отправлена</h1><p>Эскизы и ответы сохранены в проекте.</p><button className="primary-button" onClick={onBack}>Вернуться к проектам</button></section></main>
  ...
  if (!active) return <main className="questionnaire-shell"><section className="questionnaire-card"><h1>{current.title}</h1><p>Подготавливаем следующий шаг…</p></section></main>

  const options = availableOptions(active)
  const currentValue = objectAnswers[active.id]
  const canSkip = active.skip_default !== null || !active.required
  ...
  <div className="questionnaire-actions">{active.kind === 'multi' && <button className="primary-button" disabled={active.required && !multi.length} onClick={() => void answer(active, multi)}>Продолжить</button>}{active.kind === 'number' && <button className="primary-button" disabled={!draft || Number.isNaN(Number(draft))} onClick={() => void answer(active, Number(draft))}>Продолжить</button>}{active.kind === 'text' && <button className="primary-button" disabled={active.required && !draft.trim()} onClick={() => void answer(active, draft.trim())}>Продолжить</button>}{canSkip && active.kind !== 'consent' && <button className="secondary-button" onClick={() => void answer(active, active.skip_default ?? (active.kind === 'multi' ? [] : ''))}>Пропустить</button>}</div></section></div></main>
}
