...
test('shared idea deep-link opens exact work and saves on the server',async({page})=>{
  ...
  await expect(card.locator('.idea-modern-stage')).toBeVisible()
  await expect(card.locator('.idea-work-stage')).toHaveCount(0)
  await expect(card.getByRole('button',{name:'Создать с такими объектами'})).toBeVisible()
