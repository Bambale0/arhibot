...
test('admin AI history survives reload and an older still can launch a bird flyover GIF', async ({ page }) => {
  ...
  await expect(page.getByLabel('мс / кадр')).toHaveValue('90')
  await expect(page.getByText(/0 video-вызовов/)).toBeVisible()
  await expect(page.getByRole('button',{name:'Собрать 360° loop'})).toHaveCount(0)

  await page.getByRole('button',{name:'Сгенерировать flyover GIF'}).click()
