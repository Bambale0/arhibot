...
class AdminService:
    ...
    async def create_ai_flyover_gif_generation(
        ...
        if source is None:
            raise AppError(
                type="flyover_gif_source_not_found",
                title="Flyover GIF source not found",
                status=404,
                detail="The source generation does not exist or is not available.",
            )
        if source.status != GenerationStatus.COMPLETED or source.output_asset_id is None:
            raise AppError(
                type="flyover_gif_source_not_ready",
                title="Flyover GIF source is not ready",
                status=409,
                detail="Complete an AI Sandbox image before building a flyover GIF.",
            )
        if source.origin != GenerationOrigin.ADMIN_SANDBOX.value:
            raise AppError(
                type="flyover_gif_source_not_sandbox",
                title="Flyover GIF source must be an AI Sandbox image",
                status=422,
        ...
        if project is None or not bool((project.context or {}).get("admin_ai_sandbox")):
            raise AppError(
                type="flyover_gif_source_not_sandbox",
                title="Flyover GIF source must be an AI Sandbox result",
                status=422,
                detail="Use a completed result from the admin AI Sandbox.",
            )
        ...
        if len(envelope) > 12_000:
            raise AppError(
                type="flyover_gif_payload_too_large",
                title="Flyover GIF payload is too large",
                status=422,
