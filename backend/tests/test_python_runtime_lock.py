...
def test_ci_regenerates_and_audits_the_lock() -> None:
    ...
    assert 'cp requirements.lock "${tmp_dir}/requirements.lock"' in lock_script
    assert 'cp requirements-build.lock "${tmp_dir}/requirements-build.lock"' in lock_script
    assert '--upgrade' not in lock_script
